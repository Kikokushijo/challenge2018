...
class GraphicalView(object):
    ...
    def render_stop(self):
        ...
        if self.last_update != model.STATE_STOP:
            ...
            s.fill((0, 0, 0, 128), (0, 0, *viewConst.ScreenSize)); self.screen.blit(s, (0,0))
    ...
    def undulateGameSurface(self):
        ...
        n = 1
        for x in xblocks:
            xpos = (x + (math.cos(self.anim) * math.sin(x / 760 * n * 2 * math.pi) * 20))
            for y in yblocks:
                ypos = (y + (math.cos(self.anim) * math.sin(y / 760 * n * 2 * math.pi) * 20))
